...
def handler(event, context):
    ...
    current_db_data = table.scan(AttributesToGet=['Date'])

    date_list = []
    for i in current_db_data["Items"]:
    ...
    for i in fdata["Date"]:
        if fdata["Date"][i] not in date_list:
            ...
            count +=1
            print ("Date {} has been loaded into the database. Cases {},  Deaths {}, Recovered {}".format(fdata["Date"][i],fdata["Cases"][i], fdata["Deaths"][i],  fdata["Recovered"][i]))
    print ("{} dates have been added to the database.".format(count))
